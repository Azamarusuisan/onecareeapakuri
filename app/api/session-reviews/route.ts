...
export async function POST(request: Request) {
  try {
    ...
    const { data: session } = await supabase.from("sessions").select("*").eq("id", parsed.data.session_id).single();
    ...
    if (session.status !== "completed") return NextResponse.json({ error: "完了したセッションのみレビューできます" }, { status: 400 });

    const { data, error } = await supabase
      .from("session_reviews")
      .insert({ session_id: parsed.data.session_id, reviewer_id: user.id, reviewee_id: parsed.data.reviewee_id, rating: parsed.data.rating, comment: parsed.data.comment ?? null })
      .select().single();

    if (error) {
      if (error.code === "23505") return NextResponse.json({ error: "既にレビュー済みです" }, { status: 400 });
