    ...
    <div className="min-h-dvh bg-white flex flex-col">
      ...
      <div className="flex-1 flex flex-col max-w-sm mx-auto w-full">
        ...
        <div className="px-5 space-y-2.5">
          ...
          <Button onClick={handleGoogleLogin} disabled={loading} variant="outline" size="lg" className="w-full gap-2">
          ...
          {magicLinkSent ? (
            <div className="bg-[#ecfdf5] rounded-lg p-4 text-center">
              <p className="text-[13px] font-bold text-[#059669]">認証メールを送信しました</p>
              <p className="text-[12px] text-[#666] mt-1">{email} をご確認ください</p>
            </div>
          ) : (
            <form onSubmit={handleMagicLink} className="space-y-2">
              <Input type="email" placeholder="大学のメールアドレス等" value={email} onChange={(e) => setEmail(e.target.value)} />
        ...
        {error && <p className="mt-3 text-center text-[12px] text-[#ff3b30] px-5">{error}</p>}
